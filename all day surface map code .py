...
for i in range(1, 31):  # includes june 1-30, change this
    days=str(i).zfill(2)  # ensures two-digit format
    folder=os.path.join(base_folder, f'200106{days}')    
    if not os.path.isdir(folder):
        print(f"Folder {folder} not found, skipping...")
        continue
    
    for h in range(0, 24):  # includes 23 hours (0-23z), change this
        hour=str(h).zfill(2)  # ensures two-digit format
        day=f'wrfprs_d01_2001-06-{days}_{hour}.grb'
        
        try:
            filepath=os.path.join(folder,day)
            ...
            plt.title("Accumulated Precipitation (in) - 2001-06-{days} {hour} Z", fontsize=14)
            plt.savefig(f"{output_dir}/precip_200106{days}_{hour}Z.png", dpi=150)
            plt.show()
            plt.close()
            hour.close()
          
        except Exception as e:
            print("Failed to open or plot %s : {e}" % day)